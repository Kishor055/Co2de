  ...
  useEffect(() => {
    ...
    const fetchAnalyses = async () => {
      try {
        ...
        // Only fetch analyses for the currently logged in user
        if (user) {
        ...
        setAnalyses(response.documents as unknown as AnalysisItem[]);
      } catch (err) {
    ...
    const unsubscribe = client.subscribe(
      ...
      (response) => {
        // Handle only events related to this user if logged in
        const payload = response.payload as AnalysisItem;
        if (user && payload.userId !== user.$id) return;

        if (response.events.includes("databases.*.collections.*.documents.*.create")) {
          setAnalyses((prev) => [payload, ...prev]);
        }
  ...
  if (authLoading || isLoading) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-gray-500 font-medium">Syncing with Appwrite Realtime...</p>
      </div>
    );
  }
              ...
              <div className="flex flex-col lg:flex-row lg:items-center gap-6">
                <div className="flex items-center gap-4 flex-1">
                  ...
                  <div className="min-w-0">
                    <h3 className="font-semibold truncate">{analysis.fileName}</h3>
                ...
                <div className="grid grid-cols-3 gap-6 lg:gap-12 shrink-0">
                  <div className="text-center">
                    ...
                    <p className="text-xs text-gray-500 uppercase font-medium">kWh</p>
                  </div>
                  <div className="text-center">
                    <p className="text-2xl font-bold text-emerald-500">{analysis.estimatedCO2?.toFixed(2)}</p>
                    <p className="text-xs text-gray-500 uppercase font-medium">gCO2e</p>
                  </div>
                  <div className="text-center">
              ...
              <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-800">
                <div className="flex items-start gap-2">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
                    <span className="font-semibold text-gray-900 dark:text-white">Optimization:</span> {analysis.optimization}
                  </p>
                </div>
              </div>
            </div>
